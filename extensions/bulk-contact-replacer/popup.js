...
document.getElementById('startBtn').addEventListener('click', async () => {
  ...
  chrome.runtime.sendMessage({ type: 'START_PROCESS', tabId: tab.id }, async (response) => {
    await refreshStatus();
...
document.getElementById('collectBtn').addEventListener('click', async () => {
  const statusDiv = document.getElementById('status');
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  statusDiv.textContent = 'Collecting links from all pages...';
  chrome.runtime.sendMessage({ type: 'COLLECT_LINKS_ONLY', tabId: tab.id }, async (response) => {
    if (chrome.runtime.lastError) {
      statusDiv.textContent = chrome.runtime.lastError.message;
      return;
    }
    if (response && response.ok) {
      statusDiv.textContent = `Saved ${response.count} client link(s). Choose where to save the file if prompted.`;
    } else {
      statusDiv.textContent = response?.error || 'Collect failed.';
    }
  });
});
...
document.getElementById('exportBtn').addEventListener('click', async () => {
  ...
  chrome.downloads.download({ url, filename: `client-update-logs-${Date.now()}.json`, saveAs: true });
});
